...
**That direction has now been tested once, and it does not work.** Shared-odour
...
16 seeds per arm. Score is flat at every dose. The original trace analysis
appeared to show the task change reaching the policy, with the klinokinesis
signature dropping from ~6 SE to under 2 SE; **that contrast has since been
retired.** It was measured with standard errors computed across agents *within*
one population, which understate the across-seed spread by about 4×, and at 8
seeds per arm the contrast is flat (see the hazard-stakes section). Two
candidate explanations for the null were named at the time — the budget, and an
incentive worth only ~7% of elite fitness. Both have now been tested and both
are dead.

**Neither the budget nor the incentive was the binding constraint; heritability
was.** `HAZARD_STAKES` multiplies both hazard penalties, and a 3 × 2 grid of
stakes × ambiguity at 16 seeds per cell moves nothing: no score, no component,
and no hazard exposure, which is flat against its own generation-0 control in
all six cells at 16× the price. Twenty-four generations at the most favourable
cell moves it no further, while making foraging significantly worse. The reason
is measured rather than inferred: **hazard exposure has a repeatability of ~0.01
at the episode length selection uses** (`tools/repeatability.js`), and it does
not change with the price. A price multiplies a trait's signal and its noise by
the same factor, so raising it just raises the noise floor of fitness — which is
why `toxShare` goes to 1.0 while `selection` collapses from 0.030 to 0.001.
`intake` is barely heritable either (0.012–0.047), which is a plausible common
cause of the whole run of organism-side nulls and is directly testable by
raising `spawns`. **"Make the incentive bigger" is not a lever on this system.**
...
built on it is flat**, including the ambiguity abolition above. `tools/policy-agg.js`
pools across seeds and applies the project bar. No policy contrast in this
document should be believed from within-population SEs, including the ones
