...
**That direction has now also been tried, properly controlled, and it failed
too** — see the prey-evasion wave below. A task klinokinesis structurally cannot
do (escaping a pursuer) was posed, proved solvable in this arena by an
explicitly-constructed policy reading the animal's own sensors, and made to be
the only thing selection cared about. Nothing evolved. So the live direction is
now narrower again: **the next lever is the search, not the task and not the
organism.** Truncation selection on a developmental genome is what has never
