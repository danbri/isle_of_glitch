...
const CHANNEL_CONCEPTS = {
  ...
  "picture-palace": [["Feature films", "lcgft"], ["Silent films", "lcgft"]],
  "silent-twenties": [["Feature films", "lcgft"], ["Silent films", "lcgft"]],
  "late-silents": [["Feature films", "lcgft"], ["Silent films", "lcgft"]],
  "talking-pictures": [["Feature films", "lcgft"]],
  "five-cent-cinema": [["Short films", "lcgft"], ["Silent films", "lcgft"]],
  "two-reelers": [["Short films", "lcgft"]],
...
/* by-URI doc lookups, disk-cached like everything else */
async function docOf(uri) {
  const key = "uri-" + uri.replace(/\W+/g, "_").slice(-80);
  const f = join(CACHE, key + ".json");
  try { return JSON.parse(readFileSync(f, "utf8")); } catch {}
  const j = await solr({ q: `id:"${uri}"`, rows: "1", fl: "id,broader,prefLabel_en,exactLabel,prefLabel" });
  const d = j?.response?.docs?.[0] || { id: uri, broader: [] };
  try { writeFileSync(f, JSON.stringify(d)); } catch {}
  return d;
}
const labelOf = (d) => d?.prefLabel_en?.[0] || d?.exactLabel?.[0] || d?.prefLabel?.[0];

/* Every used concept's full broader set; support(B) = how many of our
   concepts sit under B. The tree then links each node to its MOST
   SPECIFIC known parent (fewest supporters), not an arbitrary one —
   "Slapstick films" files under "Comedy films", not straight under
   "Motion pictures". Adoption iterates so intermediate levels form:
   leaf genres → families → grand-families (≤3 rounds). */
const broaderOf = new Map();  // uri → broader[]
for (const uri of usedUris.keys()) broaderOf.set(uri, (await docOf(uri)).broader || []);

const support = new Map();    // candidate parent uri → #concepts under it
for (const bs of broaderOf.values()) for (const b of bs) support.set(b, (support.get(b) || 0) + 1);

const families = {};          // adopted node uri → label
let members = new Set(usedUris.keys());
for (let round = 0; round < 3; round++) {
  const adopted = [];
  for (const [b, n] of support) {
    if (n < 2 || usedUris.has(b) || families[b]) continue;
    const label = labelOf(await docOf(b));
    if (label) { families[b] = label; adopted.push(b); }
  }
  if (!adopted.length) break;
  /* adopted nodes join the family and expose their own broader links,
     so the next round can unite THEM under grand-families */
  for (const b of adopted) {
    const bs = (await docOf(b)).broader || [];
    broaderOf.set(b, bs);
    for (const g of bs) support.set(g, (support.get(g) || 0) + 1);
    members.add(b);
  }
  console.log(`  adoption round ${round + 1}: +${adopted.length} family nodes`);
}

/* link every node (concept or family) to its most specific known parent */
const known = (u) => usedUris.has(u) || families[u];
const tree = {};
let edges = 0;
for (const uri of members) {
  const cands = (broaderOf.get(uri) || []).filter((b) => b !== uri && known(b));
  if (!cands.length) continue;
  cands.sort((a, b2) => (support.get(a) || 0) - (support.get(b2) || 0));
  tree[uri] = cands[0];
  edges++;
}
/* break any accidental cycles (a↔b broader loops exist in the wild) */
for (const u of Object.keys(tree)) {
  const seen = new Set([u]);
  let cur = tree[u];
  while (cur) {
    if (seen.has(cur)) { delete tree[u]; edges--; break; }
    seen.add(cur); cur = tree[cur];
  }
}
console.log(`  ${edges} broader edges, ${Object.keys(families).length} adopted nodes`);
const depths = Object.keys(tree).map((u) => { let d = 0, c = u; while (tree[c]) { d++; c = tree[c]; } return d; });
console.log(`  max depth ${Math.max(0, ...depths)}, mean ${(depths.reduce((a, b) => a + b, 0) / (depths.length || 1)).toFixed(1)}`);
